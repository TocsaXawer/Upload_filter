...
end_path_documents = "static/Function/Documents"
...
class documents_select():
    filles_docu = fnmatch.filter(os.listdir(start_path), "*.doc")
    filles_docu1 = fnmatch.filter(os.listdir(start_path), "*.docx")
    filles_docu2 = fnmatch.filter(os.listdir(start_path), "*.pdf")

    docu_db = len(filles_docu) + len(filles_docu1) + len(filles_docu2)
pass
...
def folyam():
    ...
    for i in range(len(read.mapa)):
        m = read.mapa[i]
        x = documents_select.filles_docu
        for i in range(len(x)):
            y = (x[i])
            if m  == y:
                x = start_path + m
                shutil.move(x, end_path_documents)

    for i in range(len(read.mapa)):
        m = read.mapa[i]
        x = documents_select.filles_docu1
        for i in range(len(x)):
            y = (x[i])
            if m  == y:
                x = start_path + m
    ...
    for i in range(len(read.mapa)):
        m = read.mapa[i]
        x = documents_select.filles_docu2
        for i in range(len(x)):
            y = (x[i])
            if m  == y:
                x = start_path + m
